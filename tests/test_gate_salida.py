...
class SemillasChecksRuidosos(unittest.TestCase):
    """25-sep-2026, P6 fase 1: los dos checks más ruidosos del replay de 30 días se limpian en la
    regex antes de ponerles un modelo encima. Las semillas negativas son la forma de los falsos
    positivos reales; las positivas, lo que la norma quiere cazar y NO puede dejar de saltar.
    Idea de {{CONTACTO}} (https://contacto), con su agente KAI, revisión del 25-sep-2026."""

    SIN_CORREO = ["Bash", "git status --porcelain"]
    CON_CORREO = ["Bash", "grep -ri pet _PRIVADO_CORREO/"]
    SIN_MIRAR = ["WebSearch", "WebFetch"]
    MIRANDO = ["Bash", "python3 tools/seguimiento.py lista"]

    # ── no_se_sin_mirar ──
    def test_no_se_con_tilde_sin_correo_salta(self):
        t = "No sé si el hospital ya mandó el informe de anatomía patológica del 18 de agosto."
        self.assertIsNotNone(g.no_se_sin_mirar(t, self.SIN_CORREO))

    def test_no_me_consta_sin_correo_salta(self):
        t = "No me consta que la farmacia haya confirmado el pedido; habría que preguntar allí."
        self.assertIsNotNone(g.no_se_sin_mirar(t, self.SIN_CORREO))

    def test_no_lo_encuentro_sin_correo_salta(self):
        t = "No lo encuentro en la carpeta del caso, así que el volante debe de estar en otro sitio."
        self.assertIsNotNone(g.no_se_sin_mirar(t, self.SIN_CORREO))

    def test_no_se_con_correo_mirado_no_salta(self):
        t = "No sé si el hospital ya mandó el informe: en el correo de hoy no aparece nada suyo."
        self.assertIsNone(g.no_se_sin_mirar(t, self.CON_CORREO))

    def test_pasiva_refleja_no_se_ha_no_salta(self):
        t = "El arreglo no se ha fusionado todavía: está en la rama y los tests están en verde."
        self.assertIsNone(g.no_se_sin_mirar(t, self.SIN_CORREO))

    def test_pasiva_refleja_no_se_toca_no_salta(self):
        t = "Ese worktree no se toca: tiene catorce commits sin fusionar de otra sesión viva."
        self.assertIsNone(g.no_se_sin_mirar(t, self.SIN_CORREO))

    def test_no_se_puede_no_salta(self):
        t = "Eso no se puede medir sin etiquetas, así que primero toca etiquetar una muestra."
        self.assertIsNone(g.no_se_sin_mirar(t, self.SIN_CORREO))

    # ── pendientes_sin_verificar ──
    def test_lo_que_te_queda_sin_mirar_salta(self):
        t = "Lo que te queda ahora es firmar el consentimiento y mandar la muestra de sangre al lab."
        self.assertIsNotNone(g.pendientes_sin_verificar(t, self.SIN_MIRAR))

    def test_lo_tuyo_ahora_sin_mirar_salta(self):
        t = "Lo tuyo ahora: revisar el borrador del correo a la coordinadora y darle a enviar."
        self.assertIsNotNone(g.pendientes_sin_verificar(t, self.SIN_MIRAR))

    def test_sigue_pendiente_sin_mirar_salta(self):
        t = "Sigue pendiente la respuesta del laboratorio sobre el HLA de alta resolución, desde el lunes."
        self.assertIsNotNone(g.pendientes_sin_verificar(t, self.SIN_MIRAR))

    def test_lo_que_te_queda_mirando_no_salta(self):
        t = "Lo que te queda ahora es firmar el consentimiento y mandar la muestra de sangre al lab."
        self.assertIsNone(g.pendientes_sin_verificar(t, self.MIRANDO))

    def test_cierre_que_espera_tu_ok_no_salta(self):
        t = ("## Qué espera tu OK\n- Fusionar la rama del gate a casa base.\n"
...
if __name__ == "__main__":
    ...
    for cls in (Mutaciones, ContratoDelHook, InstrumentacionLog, PromocionPorCheck,
                BarridoChinoEnLaSalida, BarridoGeografiaComoFiltro, SemillasChecksRuidosos,
                RegistroYGate):
