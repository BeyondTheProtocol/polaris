...
def main():
    ...
    # En subproceso `-c`: desde ESTE proceso (un tests/test_*.py) gasto ya no escribe en el ledger
    # real — ver el bloque 6b —, así que lo que ve una tool de verdad se mira desde fuera.
    def _rutas(entorno):
        env = {k: v for k, v in os.environ.items() if k not in ("BTP_GASTO_LEDGER", "BTP_REPO")}
        env.update(entorno)
        rr = subprocess.run(
            [sys.executable, "-c", "import sys;sys.path.insert(0,%r);import gasto,coste;"
             "print(gasto._ledger());print(coste._ledger_path())" % TOOLS],
            capture_output=True, text=True, env=env)
        return rr.stdout.split()

    with tempfile.TemporaryDirectory() as tmp:
        os.makedirs(os.path.join(tmp, "tools"))
        r6 = _rutas({"BTP_REPO": tmp})
        ok(r6[:1] == [os.path.join(tmp, "tools", ".gasto_ledger.jsonl")],
           "gasto.py escribe en el ledger de casa base (BTP_REPO), no en el suyo")
        ok(len(r6) == 2 and r6[0] == r6[1],
           "coste.py lee exactamente el mismo fichero que gasto.py escribe")

    # --- 6b. Un TEST nunca escribe en el ledger real (25-sep-2026) -----------
    # test_perplexity_agent dejaba su simulacro (10 tokens) en el ledger de casa base.
    saved = os.environ.pop("BTP_GASTO_LEDGER", None)
    try:
        real = _rutas({})[0]
        ok(gasto._ledger() != real and "claudecode" not in gasto._ledger(),
           "desde un tests/test_*.py, gasto.py NO apunta al ledger real")
        ok(real == os.path.join(os.path.expanduser("~/claudecode"), "tools", ".gasto_ledger.jsonl"),
           "fuera de un test, el ledger sigue siendo el de casa base")
    finally:
        if saved is not None:
            os.environ["BTP_GASTO_LEDGER"] = saved
