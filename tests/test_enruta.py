...
class LasConsultasSeMidenComoBusqueda(unittest.TestCase):
    ...
    def setUp(self):
        # Sin overlay (CI, un clon) la consulta vuelve entera al borde estricto: se simula uno con
        # un lugar ficticio para que estos casos midan el código y no el entorno.
        import borde
        self._seg = borde.seg
        self._lugares = self._seg._LUGARES_RUTA
        if not self._lugares:
            self._seg._LUGARES_RUTA = frozenset({"villaficticia"})

    def tearDown(self):
        self._seg._LUGARES_RUTA = self._lugares

    def test_persona_descrita_o_lugar_de_su_ruta_no_va_a_grok(self):
        """22-sep-26: huecos (a) y (b) de `verificacion` sobre b9d6c29."""
        self._seg._LUGARES_RUTA = self._seg._LUGARES_RUTA | {"villaficticia"}
        for q in ("paciente que busca vacuna personalizada en Villaficticia biopsia",
                  "busca en X la vacuna de neoantígenos para una paciente de mama"):
            d = enruta.elegir(q, estado=TODOS_VIVOS)
            self.assertTrue(d.sensible, q)
            self.assertNotIn("grok", d.elegidos)
            self.assertNotIn("perplexity", d.elegidos)

    def test_sin_overlay_de_lugares_falla_cerrado(self):
        self._seg._LUGARES_RUTA = frozenset()
        d = enruta.elegir("busca en X qué se dice de la vacuna de BioNTech", estado=TODOS_VIVOS)
        self.assertTrue(d.sensible, "sin lista de lugares el embargo tiene que volver")
