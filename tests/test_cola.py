...
def main():
    ...
    # ── la intención no se pierde al encolar (20-sep-2026) ───────────────────────────────
    # `enqueue` cogía args[-1]: una llamada acabada en flag encolaba el VALOR del flag. Dos jobs
    # reales se ejecutaron con la intención literal «exec» y costaron 0,24 y 0,36 USD.
    texto = "revisa el latido del correo y escribe lo que encuentres"
    q.main(["enqueue", texto, "--agente", "tecnico", "--procedencia", "radar-taller", "--tipo", "exec"])
    ultimo = sorted(os.listdir(os.path.join(q.QUEUE, "pending")))[-1]
    with open(os.path.join(q.QUEUE, "pending", ultimo), encoding="utf-8") as fh:
        encolado = json.load(fh)
    check("la intención sobrevive a los flags de detrás", encolado["intencion"] == texto)
    check("y los flags se siguen leyendo", encolado["agente"] == "tecnico" and encolado["tipo"] == "exec")

    q.main(["enqueue", "--agente", "tecnico", texto])
    ultimo2 = sorted(os.listdir(os.path.join(q.QUEUE, "pending")))[-1]
    with open(os.path.join(q.QUEUE, "pending", ultimo2), encoding="utf-8") as fh:
        check("da igual el orden: el texto es el primer posicional",
              json.load(fh)["intencion"] == texto)

    for basura in ("exec", "privileged", "alta", "critico", "   "):
        try:
            q.enqueue(basura, procedencia="test")
            check("enqueue(%r) tenía que fallar" % basura, False)
        except ValueError:
            check("enqueue rechaza la intención %r (es el valor de un flag)" % basura, True)
    check("una intención corta y legítima SÍ se encola (Telegram escribe corto)",
          bool(q.enqueue("¿cómo va?", procedencia="telegram:triado")))

    # ── `prueba`: qué tiene que quedar escrito para poder cerrar el job (20-sep-2026) ────
    jid = q.enqueue("deja una nota con lo que encuentres", procedencia="t",
                    prueba={"tipo": "ruta", "ruta": "04 · IA/Notas/algo.md"})
    check("un job con prueba válida se encola", bool(jid))
    for mala, etiqueta in (({"tipo": "commit", "rama": "x"}, "tipo inventado"),
                           ({"tipo": "ruta", "ruta": "tools/seguimiento.py"}, "un .py (el muro no deja escribirlo)"),
                           ({"tipo": "ruta", "ruta": "/etc/passwd"}, "ruta absoluta"),
                           ({"tipo": "ruta", "ruta": "../fuera.md"}, "ruta que se sale del repo"),
                           ({"tipo": "deuda"}, "deuda sin clave"),
                           ({"tipo": "ruta", "ruta": "x.md", "extra": 1}, "campo de más")):
        try:
            q.enqueue("intencion larga y legitima para el test", procedencia="t", prueba=mala)
            check("prueba rechazada: %s" % etiqueta, False)
        except ValueError:
            check("prueba rechazada: %s" % etiqueta, True)

    # el CLI sabe declarar la prueba (lo usan los carriles automáticos, p.ej. radar-taller)
    q.main(["enqueue", "deja una nota con el veredicto", "--procedencia", "radar-taller",
            "--prueba", '{"tipo":"deuda","clave":"radar-taller:algo"}'])
    ult = sorted(os.listdir(os.path.join(q.QUEUE, "pending")))[-1]
    with open(os.path.join(q.QUEUE, "pending", ult), encoding="utf-8") as fh:
        j = json.load(fh)
    check("--prueba llega al job", j.get("prueba") == {"tipo": "deuda", "clave": "radar-taller:algo"})
    check("y la intención no se la come el flag nuevo",
          j.get("intencion") == "deja una nota con el veredicto")
    check("--prueba con JSON roto no encola", q.main(["enqueue", "texto legitimo de prueba",
                                                     "--prueba", "{esto no es json"]) == 2)
