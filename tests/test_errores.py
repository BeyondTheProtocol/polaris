...
# ── 8) el resumen aguanta severidad/agente a None ─────────────────────────────
# 12-sep-26: un registro con la clave presente y valor None hacía que sorted() lanzara
# TypeError ('<' not supported between str and NoneType) y el resumen del CLI no se podía
# leer, el día que había 398 errores. El default de get() no salva: la clave SÍ está.
print("\n[8] _resumen no revienta con severidad o agente a None")
_leer_real = o._leer_dias
o._leer_dias = lambda dias: [{"resultado": "fail", "severidad": None, "agente": "x"},
                             {"resultado": "fail", "severidad": "alta", "agente": None}]
try:
    rc = e._resumen(1)
    ok(rc == 0, "_resumen devuelve 0 con None dentro")
except Exception as exc:
    ok(False, "_resumen lanzó: %r" % exc)
finally:
    o._leer_dias = _leer_real
