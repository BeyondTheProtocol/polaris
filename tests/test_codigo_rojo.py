...
def _no_hay_interruptor():
    """22-sep-2026: BTP_HALT_FILES NO puede cambiar dónde escribe el código rojo real (sería un
    interruptor para silenciarlo). Solo mira la constante en un proceso aparte: no llama a trigger."""
    import subprocess
    out = subprocess.run([sys.executable, "-c",
                          "import sys; sys.path.insert(0, %r); import codigo_rojo as c; print(c.HALT_FILES[0])"
                          % os.path.join(ROOT, "tools")],
                         capture_output=True, text=True,
                         env=dict(os.environ, BTP_HALT_FILES="/tmp/x_halt_a:/tmp/x_halt_b")).stdout.strip()
    check("BTP_HALT_FILES no silencia el código rojo real (HALT_FILES sigue en ~/.btp.HALT)",
          out == os.path.join(os.path.expanduser("~"), ".btp.HALT"))
