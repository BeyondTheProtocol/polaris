...
class ElBarridoChino(unittest.TestCase):
    """20-sep-2026, Bloque A. {{TITULAR}}: «busca en China también, que para eso tenemos hueco en
    servidor allí» — el carril EN SESIÓN no tenía ninguna regla escrita, solo el carril
    automático (`radar_ned_diario.TEMAS_CN`) miraba China. Esto prueba que buscar novedades o
    soluciones clínicas manda el paso de China, y que interpretar SU dato fijo (o algo sin
    comité médico) no dispara ruido."""

    def test_buscar_novedades_exige_china_con_herramienta_concreta(self):
        d = _d("busca qué se ha publicado esta semana sobre FGFR1 amplificado en mama HR+")
        self.assertTrue(d.get("china_obligatoria"))
        self.assertIn("radar_ned_diario.py run --tema cn-", d["orden"])

    def test_interpretar_su_propio_dato_no_exige_china(self):
        d = _d("el SUVmax de la lesión ósea ha bajado, qué quiere decir")
        self.assertFalse(d.get("china_obligatoria"))
        self.assertNotIn("radar_ned_diario.py run --tema cn-", d["orden"])

    def test_sin_comite_medico_la_palabra_evidencia_no_dispara_china(self):
        d = _d("necesito evidencia de que pagué la factura del hotel")
        self.assertNotIn("comite-medico", d["comites"])
        self.assertFalse(d.get("china_obligatoria"))

    def test_ensayo_clinico_dispara_china(self):
        d = _d("hay algún ensayo clínico fase 1 de vacuna personalizada en Europa para el que "
               "sea elegible?")
        self.assertTrue(d.get("china_obligatoria"))
...
if __name__ == "__main__":
    ...
    for cls in (LaDecision, ElMuroMandaSobreLaDecision, ElBarridoChino, Precision, ElCumplimiento,
                LaCajaSeConvoca):
