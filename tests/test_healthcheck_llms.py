...
class TestDistingueSaldoDeCaida(unittest.TestCase):
    ...
    def test_sin_saldo_tiene_clave_propia(self):
        claves, info = self._claves({
            "glm": {"ok": False, "detalle": 'error 429: {"message":"Insufficient balance or no resource package"}'}})
        self.assertIn("llm_sin_saldo:glm", claves)
        self.assertEqual(info["sin_saldo"], ["glm"])
    ...
    def test_sin_saldo_NO_espera_a_la_segunda(self):
        ...
        claves, _ = self._claves({"glm": {"ok": False, "detalle": "Insufficient balance"}})
        self.assertIn("llm_sin_saldo:glm", claves)
    ...
    def test_no_martillea_a_los_proveedores_en_cada_vuelta(self):
        self._claves({"glm": {"ok": False, "detalle": "Insufficient balance"}})   # deja estado
        alertas, info = hc._check_llms(salud={"glm": {"ok": False, "detalle": "Insufficient balance"}})
        self.assertEqual(alertas, [])
        self.assertTrue(info.get("throttled"))
        self.assertEqual(info.get("sin_saldo"), ["glm"])    # pero no olvida lo que sabía
