...
check(len(stamps()) == 2, "guarda el OK nuevo sin borrar el de la otra versión (uno por clave)")
...
check(len(stamps()) == 2, "no cachea OK para la versión rota")
...
check(len(stamps()) == n0, "OK de esa versión invalidado (se creó en el preflight y se borró)")
...
check(len(stamps()) == n0, "OK invalidado: el siguiente repite el preflight")
