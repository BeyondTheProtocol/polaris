...
from _entorno import exige as _exige, exige_cola_aislada as _exige_cola_aislada
_exige("estado")
import os
import sys
import tempfile
...
# Este fichero PLANTA jobs de mentira en failed/ (recover_tests). Si la cola resuelta no es la
# temporal —p.ej. porque algo importó `cola` antes de la línea de arriba—, esos fixtures acaban en
# la cola VIVA y el healthcheck los toma por jobs reales. Pasó el 20-sep-26. Aquí se corta.
_exige_cola_aislada()
...
def salud_tests():
    ...
    # La CLAVE, no solo el texto (20-sep-26): el libro de deudas indexa por clave, así que un
    # renombrado del prefijo dejaba este test verde y la deuda `daemon_asistente_fallo` huérfana.
    ok(any(a[0] == "daemon_asistente_fallo" for a in al1 if isinstance(a, tuple)),
       "la clave del fallo del asistente es exactamente 'daemon_asistente_fallo'")
...
def roster_tests():
    ...
    # Clave literal con un label REAL del roster (20-sep-26): `daemon_fallando:com.btp.hoy-compose`
    # llevaba 6 detecciones en el libro y nada fijaba que la clave se formara con el label tal cual.
    hc._daemons_roster = lambda: {"com.btp.hoy-compose"}
    hc._daemons_keepalive = lambda: set()
    hc._launchctl_estado = lambda: {"com.btp.hoy-compose": ("-", "1")}
    al_hc, _ = hc._check_roster_daemons()
    ok("daemon_fallando:com.btp.hoy-compose" in [a[0] for a in al_hc],
       "la clave lleva el label entero: daemon_fallando:com.btp.hoy-compose")
    hc._launchctl_estado = lambda: {"com.btp.hoy-compose": ("-", "0")}
    ok(not any(a[0].startswith("daemon_fallando") for a in hc._check_roster_daemons()[0]),
       "el mismo daemon con exit 0 no dice nada (control inverso)")
    hc._daemons_keepalive = lambda: {"com.btp.ka", "com.btp.instagram"}
    hc._daemons_roster = lambda: {"com.btp.ka", "com.btp.iv", "com.btp.run", "com.btp.fail", "com.btp.instagram"}
