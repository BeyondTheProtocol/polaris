...
class GuardDeSalida(unittest.TestCase):
    ...
    def test_fusiona_abre_el_merge_del_pr_que_tenia_delante(self):
        """Escribió «fusiona» para el #224 y el freno no la entendió: solo sabía envía/publica."""
    ...
    def test_fusiona_sin_numero_exige_que_lo_tuviera_delante(self):
        self._yo("He terminado el informe.")
    ...
    def test_fusionar_no_abre_el_envio(self):
        ...
        self.assertEqual(self._bash("cd /Users/polaris/projects/.mgc-staging/x && git push origin main"),
                         "deny")
        self.assertEqual(self._decision(self._hook({"tool_name": self.PROGRAMA, "tool_input": {}})),
                         "deny")
        self.assertIsNone(self._bash(self.MERGE_WEB % 224), "y sigue intacto para lo que sí pidió")

    def test_no_es_orden_de_fusionar(self):
        for texto in ("no lo fusiones todavía", "¿fusionamos luego?", "la fusión de ayer",
    ...
    def test_el_aviso_dice_fusionar(self):
        ...
        self.assertIn("FUSIONAR", r.stdout)

    def test_permiso_de_otra_sesion_explica_que_frase_lo_abre(self):
        """25-sep: «el permiso es de otra sesión» sonaba a que su orden llegó y se atribuyó mal."""
        self._yo("¿fusiono el #224?")
        self._ok_envio("fusiona")
        payload = {"tool_name": "Bash", "cwd": self.WT, "permission_mode": "bypassPermissions",
                   "session_id": "otra-sesion", "tool_input": {"command": self.MERGE_WEB % 224}}
        r = self._hook(payload)
        self.assertEqual(self._decision(r), "deny")
        self.assertIn("no abrió permiso", r.stdout)
        self.assertIn("fusiónalo", r.stdout)
