...
class TestNoPublicarDelOverlay(unittest.TestCase):
    """Lo que el overlay local declara privado no entra en el árbol público, y ningún
    fichero publicable lleva dentro los marcadores que el overlay dice vigilar."""

    def _overlay(self):
        try:
            with io.open(publicar._overlay("perfil.local.json"), encoding="utf-8") as fh:
                return json.load(fh)
        except (OSError, ValueError):
            self.skipTest("sin overlay local (árbol público): no hay lista que probar")

    def test_lo_declarado_privado_no_entra(self):
        privados = self._overlay().get("no_publicar") or []
        if not privados:
            self.skipTest("el overlay no declara ficheros privados")
        for rel in privados:
            self.assertFalse(publicar._entra(rel), "%s entraría en el árbol público" % rel)

    def test_ningun_fichero_publicable_lleva_los_marcadores(self):
        marcadores = self._overlay().get("marcadores_privados") or []
        if not marcadores:
            self.skipTest("el overlay no declara marcadores")
        raiz = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        fugas = []
        for rel in publicar.versionados():
            if not publicar._entra(rel):
                continue
            try:
                ...
                continue
            fugas += ["%s: %s…" % (rel, m[:3]) for m in marcadores if m.lower() in txt.lower()]
        self.assertEqual(fugas, [], "marcadores privados en ficheros publicables")
