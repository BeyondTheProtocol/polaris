...
class TestMuro(unittest.TestCase):
    ...
    def test_la_cara_publica_reserva_los_comites_delatores(self):
        """El scrubber caza léxico, no contexto: 'un comité' no dice nada
        prohibido y aun así delata que hay un asunto privado."""
        html = anatomia.render(cara="publica")
    ...
    def test_la_cara_publica_tacha_contactos_como_el_repo(self):
        """El panel no puede enseñar el nombre de un contacto que el repo público
        ya tacha (22-sep-26: «{{CONTACTO}}», «{{CONTACTO}}» y «{{CONTACTO}}» salían en los tooltips)."""
        terceros = anatomia._nombres_de_terceros()
        if not terceros:
            self.skipTest("sin overlay nombres.local.json: no hay deny-list que probar")
        n = terceros[-1]
        self.assertEqual(anatomia.limpiar("%s: se le CONSULTA" % n.capitalize(), "publica"),
                         "[contacto]: se le CONSULTA")
        self.assertEqual(anatomia.limpiar("%s: se le CONSULTA" % n, "privada"),
                         "%s: se le CONSULTA" % n)
        html = anatomia.render(cara="publica").lower()
        for t in terceros:
            self.assertNotRegex(html, r"(?<!\w)%s(?!\w)" % re.escape(t.lower()),
                                "la cara pública nombra al contacto %r" % t)
