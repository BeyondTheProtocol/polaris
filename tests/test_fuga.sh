...
# Detector de la invariante B3, compartido por el barrido y su regresión de abajo.
habla_directo() {
  grep -E 'api\.telegram\.org/bot|/sendMessage\b|/messages\b' "$1" \
    | grep -vq 'https://api\.anthropic\.com/v1/messages'
}
inv_fail=0
for f in "$ROOT"/tools/*.py; do
  ...
  # La línea del endpoint de Claude (api.anthropic.com/v1/messages, `reescribe_consulta.py`,
  # 22-sep-26) no es un envío a {{TITULAR}}: es el carril seguro. Se excluye esa línea exacta, no el
  # fichero: un /sendMessage o un /messages de IG en el mismo fichero seguiría cantando.
  if habla_directo "$f"; then
...
# Regresión del detector (deuda test-fuga-falso-positivo-api-anthropic, 22-sep-26): la
# exclusión de la línea de Claude no puede apagar lo que el detector está para cazar.
# NO se ancla /messages al host de Meta: en salida.py el host va en una variable
# ("%s/%s/messages" % (IG_GRAPH, igid)) y el anclaje dejaría pasar el envío real de DMs.
_DET_DIR="$(mktemp -d)"
det() { # esperado(CAZA|PASA)  nombre  contenido
  printf '%s\n' "$3" > "$_DET_DIR/$2.py"
  local got=PASA; habla_directo "$_DET_DIR/$2.py" && got=CAZA
  if [ "$got" = "$1" ]; then pass=$((pass+1));
  else fail=$((fail+1)); printf '  ✗ [detector] esperaba %-4s obtuvo %-4s :: %s\n' "$1" "$got" "$2"; fi
}
det PASA claude_api   'URL = "https://api.anthropic.com/v1/messages"'
det CAZA tg_bot       'u = "https://api.telegram.org/bot%s/getMe" % tok'
det CAZA tg_send      'r = requests.post(base + "/sendMessage", data=d)'
det CAZA ig_dm_var    'url = "%s/%s/messages" % (IG_GRAPH, igid)'
det CAZA ig_dm_host   'url = "https://graph.facebook.com/v21.0/123/messages"'
det CAZA mezcla       "$(printf 'URL = "https://api.anthropic.com/v1/messages"\nurl = "%%s/%%s/messages" %% (IG_GRAPH, igid)')"
det CAZA mezcla_tg    "$(printf 'URL = "https://api.anthropic.com/v1/messages"\nrequests.post(b + "/sendMessage")')"
rm -rf "$_DET_DIR"
