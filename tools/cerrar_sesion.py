...
import _lock  # noqa: E402 — candado COMPARTIDO "git-mutex" (git_mutex.py) para serializar
              # TODA mutación del `.git` de la casa base (fusión Y poda), no solo la fusión.
GIT_MUTEX = "git-mutex"
...
def cerrar(apply=False, scope=None, podar=True):
        ...
        if apply:
            ...
            try:
                with _lock.lock(GIT_MUTEX, timeout=60.0):
                    ...
                    if rc != 0:
                        return dict(p, error="fusión falló (conflicto/estado): %s" % err, acciones=acciones)
                    fusionado = True
    ...
    if p["docs_nuevos"]:
        if apply:
            ...
            rc, out, err = _run(["python3", os.path.join(BASE, "tools", "kb.py"), "index"], cwd=BASE)
