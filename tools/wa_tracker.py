...
# 21-jun-2026: un dep de Whisper (`from queue import Queue`) cazaba `tools/queue.py` —la cola del
# lazo— en vez del `queue` estándar, y la transcripción se caía. Se arregló quitando tools/ del
# path. 20-sep-2026: eso ya no hace falta y hacía daño. `tools/queue.py` se renombró a
# `tools/cola.py` el 11-jul-26, así que no queda nada que tape la stdlib (barrido contra
# sys.stdlib_module_names: cero). En cambio la purga borraba la entrada del LLAMADOR: el bot de
# Telegram importa wa_tracker en caliente y se quedaba sin tools/ en el path para el resto de su
# vida. Que no vuelva a haber sombra lo vigila `tests/test_tools_no_sombrea_stdlib.py`.
