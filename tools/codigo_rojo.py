...
# NO respeta BTP_HALT_FILES, y es A PROPÓSITO (22-sep-2026). El borde sí la lee para AISLAR sus
# tests; aquí sería un interruptor: cualquier proceso que la definiera convertiría el código
# rojo real en un simulacro. La protección contra un disparo de prueba está en quien prueba
# (espía de `trigger`, ficha de `verificacion`) y en que un canario de prueba ya no escala
# (`borde.canario_y_origen`). `tests/test_codigo_rojo.py` fija esta decisión.
