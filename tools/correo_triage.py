...
def _espera_firma(m):
    ...
    # …y que no sea ruido conocido (20-sep-2026): un boletín comercial de Amazon, con un alias
    # compuesto y un asunto que hablaba de «sign-up», salía aquí como si esperara su firma.
    # Mismo filtro que ya lleva `otros_urgentes` unas líneas más abajo; el arreglo va aquí y no
    # en `es_persona_real`, cuyo fail-safe hacia «persona» protege a la secretaría del próximo
    # hospital que escriba.
    #
    # La dirección concreta NO se escribe aquí: `ci_barrido.py` la marcó como correo personal y
    # dejó el CI del repo público en rojo. Da igual que sea una dirección corporativa y no la de
    # nadie —el barrido no puede distinguirlo, y hace bien en no intentarlo—. El detalle vive en
    # el libro de deuda, que no se publica. Misma clase que el veto escrito en el propio código.
...
def resumen(cuentas=None):
    ...
    # El cubo rojo es correspondencia clínica REAL: un PR titulado «Biopsia ósea» no entra por
    # llevar la palabra (10 de los 29 NED-críticos del buzón eran de github.com, 20-sep-26). Se
    # DEGRADA aquí, en el consumo; `puede_archivar` sigue viéndolo NED-crítico y no lo archiva.
    ned = [m for m in urgentes if m.get("ned_critico")
           and not correo.es_ci_dev(m.get("remitente_email", ""))]
