...
def _lineas_ya_en(src, dst):
    ...
    Para los logs y paneles que CRECEN por abajo: un worktree se lleva una copia al nacer, casa
    base sigue escribiendo, y luego el fichero del worktree es un prefijo del de casa base. Byte a
    byte no son iguales, pero no hay nada que salvar. Lo usa `_motivo_residuo`.
    (24-sep-26: esta función y el rescate de otra sesión se cruzaron en dos ramas a la vez; se
    quedó el rescate de allí y este ayudante de aquí, que es lo que aquel empezó a usar.)"""
    try:
