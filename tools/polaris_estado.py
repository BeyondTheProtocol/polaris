...
def _importar_de_casa_base(nombre):
    """Importa un hermano de casa base DEJANDO el sys.path como estaba.

    Aqui habia un `sys.path.insert(0, TOOLS)` a nivel de modulo, con un comentario honesto
    («para poder importar salida sin depender del cwd»). El import que lo necesitaba es uno
    solo y es PEREZOSO (dentro de main, tras `--send`), pero el efecto era global y permanente:
    desde un worktree, `<casa base>/tools` quedaba al FRENTE del sys.path del proceso, y todo
    lo que se importara despues -por cualquiera- resolvia a CASA BASE en vez de a la rama.

    Como se vio (20-sep-2026): healthcheck._check_roster_daemons() importa este modulo, y a
    partir de ahi un `import seguimiento` daba el fichero de casa base. Un test de la rama se
    quedaba probando el codigo de casa base y pasaba en VERDE — verificar antes de fusionar era
    teatro, igual que el bug que tests/test_clinico_guard.py ya documenta, pero invisible porque
    no hay ninguna ruta escrita a mano: solo un sys.path que alguien dejo torcido.

    Mismo patron que healthcheck._importar_sin_tocar_path. Lo que este modulo mide sigue siendo
    casa base, que es su razon de ser; lo que ya no hace es decidirselo al resto del proceso.
    """
    import importlib
    antes = list(sys.path)
    try:
...
def main():
    ...
    if "--send" in sys.argv:
        try:
            salida = _importar_de_casa_base("salida")
