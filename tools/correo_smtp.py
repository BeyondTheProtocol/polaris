...
# 20-sep-2026: aquí había un `sys.path = [p for p in sys.path if … != _tools_dir]` que borraba
# tools/ del path del PROCESO ENTERO, no solo la entrada que este fichero añadía. Llegó copiado
# de drive.py para evitar que `tools/queue.py` tapara la `queue` de la stdlib… y ese fichero se
# renombró a `tools/cola.py` el 11-jul-26. La defensa sobrevivió al peligro y causó el daño:
# healthcheck importaba esto y sus imports perezosos posteriores fallaban — detector de frescura
# ciego 74 días, 1.334 fallos. Quien vigila que no vuelva a haber sombra es
# `tests/test_tools_no_sombrea_stdlib.py`, en la causa y no en el síntoma.
