...
while true; do
  ...
  if ! "$PY" "$TOOLS/cost_guard.py" check >/dev/null 2>>"$LOG/dispatcher.err"; then
    ...
    log "cost_guard: sin saldo de pago → pauso rutina 60s"; [ -n "${BTP_ONCE:-}" ] && break; sleep 60; continue
  fi
