...
def _titular(clave):
    ...
    """
    ruta = os.path.join(ROOT, "tools", "perfil.local.json")
    try:
...
def _nombres_de_terceros():
    ...
    ruta = os.path.join(ROOT, "tools", "nombres.local.json")
    try:
        with io.open(ruta, encoding="utf-8") as fh:
            d = json.load(fh)
    except (OSError, ValueError):
    ...
    nombres = {str(x) for x in d.get("nombres", []) if len(str(x)) >= 4}
