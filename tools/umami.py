...
Forma de la API (api.umami.is/v1), verificada 2026-06-20:
  ...
  ⚠️ FILTRAR POR PÁGINA ES `path=`, **NUNCA `url=`** (comprobado 20-sep-2026):
       `&url=/lo-que-sea` se IGNORA en silencio y devuelve el sitio ENTERO — incluso
       con una ruta inventada. Se creyó medir una página y eran las cifras globales.
       Usa `stats_pagina()` de aquí abajo, que ya manda `path=` y lo verifica.
...
def stats_pagina(path, desde_ms, hasta_ms=None, key=None, wid=None):
    """Stats de UNA página. Filtra con `path=` (ver aviso del docstring: `url=` NO filtra).
    -> dict {vistas, visitantes, visitas, rebotes, tiempo_total, seg_por_visita}.
    Lanza RuntimeError si el filtro no está recortando nada, que es la señal de que la
    API ignoró el parámetro y estaríamos dando el total del sitio como si fuera la página."""
    if key is None:
        key = get_secret("btp-umami-api", SECRETS, "api_key")
    if not key or str(key).startswith("PEGA"):
        raise RuntimeError("falta la clave de Umami (Llavero btp-umami-api)")
    wid = _wid_de_secrets(wid)
    hasta_ms = hasta_ms or int(time.time() * 1000)
    rango = "startAt=%d&endAt=%d" % (desde_ms, hasta_ms)
    pag = api("/websites/%s/stats?%s&path=%s" % (wid, rango, path), key)
    # Fail-closed: si filtrar da EXACTAMENTE lo mismo que no filtrar, el filtro no se aplicó.
    todo = api("/websites/%s/stats?%s" % (wid, rango), key)
    if stat_num(pag, "pageviews") == stat_num(todo, "pageviews") and stat_num(todo, "pageviews"):
        raise RuntimeError(
            "el filtro por página no recortó nada (%s vistas = el sitio entero): "
            "la API ignoró `path`; NO uses estas cifras como si fueran de %s" % (
                stat_num(todo, "pageviews"), path))
    vis = stat_num(pag, "visits") or 0
    tt = stat_num(pag, "totaltime") or 0
    return {"vistas": stat_num(pag, "pageviews"), "visitantes": stat_num(pag, "visitors"),
            "visitas": vis, "rebotes": stat_num(pag, "bounces"), "tiempo_total": tt,
            "seg_por_visita": round(tt / vis) if vis else None}
