...
# ── barrido chino obligatorio (20-sep-2026, Bloque A) ──────────────────────────────────────────
# {{TITULAR}}, 18-sep-26: «busca en China también, que para eso tenemos hueco en servidor allí». El
# carril AUTOMÁTICO (`radar_ned_diario.TEMAS_CN`) ya mira China; el carril EN SESIÓN no tenía
# ninguna regla escrita — la petición vivía solo como comentario en ese fichero. Dispara sobre el
# vocabulario de BUSCAR novedades o soluciones (novedad, tratamiento, ensayo, evidencia, paper),
# no sobre cualquier tema clínico: interpretar SU informe o SU biomarcador fijo no es investigar
# mundo afuera, y disparar ahí sería ruido sin más precisión. Solo se evalúa cuando `comite-medico`
# ya entra por la clasificación de `enruta_comite` (evita falsos positivos fuera de lo clínico:
# "tratamiento fiscal de las donaciones" no activa comité médico, así que tampoco esto).
INVESTIGACION_CN = re.compile(
    r"\bnovedad\w*|qu[ée] hay (de )?nuevo|[uú]ltim\w+ (avance|estudio|publicaci\w*|investigaci\w*|"
    r"semana)|qu[ée] se (ha publicado|sabe|dice)|\bevidencia\b|\bensayo\w*\b|\btrial\w*\b|"
    r"\bpapers?\b|\bliteratura\b|\btratamientos?\b|\bterapias?\b|\bf[áa]rmac\w*|"
    r"opci[oó]n(es)? de tratamiento|qu[ée] (opciones|hay) (contra|para)\b",
    re.I)
...
def decidir(prompt, *, contexto_clinico=False, sesion_sensible=None, estado=None):
    ...
    if "comite-medico" in comites and INVESTIGACION_CN.search(texto):
        out["china_obligatoria"] = True
        out["motivos"].append("{{TITULAR}}, 18-sep-26: toda investigación de novedades incluye China")
...
def orden(d):
    ...
    if d.get("china_obligatoria"):
        pasos.append(
            "Bash: python3 tools/radar_ned_diario.py run --tema cn-adc (o cn-celular / cn-inmuno "
            "/ cn-dianas, el que encaje con la diana) — si ninguno encaja, Europe PMC a mano con "
            "AFF:\"China\" + tools/cn_fetch.py contra el CDE de la NMPA o ChiCTR/ICTRP. Declara al "
            "cierre qué fuente china se abrió y cuál no: nunca se investiga sin mirar China.")
...
def evaluar(ruta):
    ...
    for ln in open(ruta, encoding="utf-8"):
        ...
        if "china" in c and bool(d.get("china_obligatoria")) != bool(c["china"]):
            mal.append("china %s (esperado %s)" % (bool(d.get("china_obligatoria")), c["china"]))
        if mal:
