...
GLOSARIO = [
    ...
    ("el latido de fuera", "dead-man switch externo",
     "un ping vacío cada 30 min a Healthchecks.io; si deja de llegar, avisa alguien que no vive en el Mac"),
]
...
CAMINO = [
    ...
    ("el subagente", "tools/run_agent.sh",
     ...
     "si no los cargó, no arranca y dispara el código rojo. Corre una versión "
     "fija del arnés, que solo sube cuando la nueva pasa ese canario."),
    ("los guardarraíles", ".claude/hooks/",
     "Antes de CADA herramienta corre un hook que puede denegarla: en el lazo, una "
     "allowlist fail-closed; en interactivo, el guarda de lo clínico. Un guard que "
     "tarda más que su plazo deniega él mismo (el arnés, si lo cancela, dejaría pasar), "
     "y debajo hay reglas de permiso que el arnés aplica aunque el hook no conteste: el "
     "código del muro en casa base y las zonas clínicas. Y al cerrar "
...
FAMILIAS = [
    ("muro y seguridad", ("muro", "codigo_rojo", "seguridad", "_lexico", "deid",
                          ...
                          "canarios",
                          # canario del muro: el arnés cargó los hooks, y la versión fija del
                          # lazo que solo sube con él en verde (P9, idea de {{CONTACTO}} + KAI)
                          "canario_preflight", "claude_lazo",
...
def _listas(inv, pul, cara):
    ...
    if mq:
        ...
        bloque("red", "y cómo se llega hasta aquí",
               ['<span class="chip">nada escucha desde internet</span>',
                '<span class="chip">el panel se EMPUJA, cifrado</span>',
                '<span class="chip">la clave no está en el servidor</span>'],
               "no hay ningún puerto ni túnel público hacia esta máquina. Lo que se "
               "ve desde fuera de casa va por su red privada (Tailscale), solo para "
               "sus aparatos, y la lista cerrada de lo que escucha la vigila un test. "
               "Lo que se publica sale de aquí ya cerrado: quien lo sirve no sabe "
               "abrirlo, y quien lo abre es el navegador de quien tiene la frase")
