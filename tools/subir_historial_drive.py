...
def _ruta_identidad():
    """El overlay del repo actual, y si no está, el de casa base.

    Es gitignored (son las marcas del titular), así que no viaja a un worktree — y ahí este
    módulo se quedaba SIN reconocimiento positivo: no bloqueaba nada mal, pero dejaba de
    comprobar que el historial es de ella, que es justo su razón de existir. Mismo arreglo que
    `seguimiento._cargar_nombres_deny` y `tests/_entorno._hay_overlay` (20-sep-26)."""
    aquí = os.path.join(os.path.dirname(os.path.abspath(__file__)), "identidad.local.json")
    if os.path.isfile(aquí):
        return aquí
    base = os.path.join(os.environ.get("BTP_REPO") or os.path.expanduser("~/claudecode"),
                        "tools", "identidad.local.json")
    return base if os.path.isfile(base) else aquí


IDENTIDAD = _ruta_identidad()
...
def _texto(ruta):
    if ruta.lower().endswith(".pdf"):
        ...
        # Las DOS convenciones de nombre del sidecar (20-sep-26). Aquí solo se miraba
        # `X.ocr.txt`, y el OCR que de verdad genera el pipeline se llama `X.pdf.ocr.txt`:
        # un PDF escaneado —que no da texto por `pdftotext`— se verificaba contra 1 o 2 bytes,
        # o sea A CIEGAS, y `es_suyo()` lo retenía sin haber leído nada. Medido sobre los 419
        # PDF del historial: 5 en ese caso, y 3 de ellos SÍ identifican a la paciente en el OCR
        # que se estaba ignorando. No relaja el criterio (sigue exigiendo fecha de nacimiento o
        # identificador): lo aplica sobre el texto que ya existía y no se leía.
        for ocr in (ruta[:-4] + ".ocr.txt", ruta + ".ocr.txt"):
