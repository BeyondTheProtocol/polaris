...
def _kickstart_daemon(label):
    """Intenta revivir un daemon con `launchctl kickstart -k gui/<uid>/<label>`.
    Solo se llama cuando el daemon tiene KeepAlive=true (launchd ya lo gestiona;
    kickstart fuerza el ciclo inmediato). Devuelve True si el comando salió con 0.
...
_BORRAN_TOOLS_DEL_PATH = ("correo_smtp", "drive", "postdicom", "wa_tracker")
...
def _ciclo_agentes():
    ...
    return {"resueltos": len(resueltos), "por_estado": por_estado,
