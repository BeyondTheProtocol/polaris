...
UMBRAL_SEG_DEFAULT = int(os.environ.get("BTP_BUCLE_UMBRAL_SEG") or 2 * 3600)
...
def _es_bucle_espera(cmd):
    return bool(_RE_BUCLE_ESPERA.search(cmd))
    ...
    if dry_run:
        ...
        try:
            ...
            _deuda_mod.visto(
                ...
                nota="parado(s) por bucles_colgados.py: %d pid(s), ej. pid %d (%s)"
                     % (len(parados), parados[0]["pid"], _redactar_truncar(parados[0]["cmd"], 60)),
            )
        except Exception:
            pass
        alertas.append((
