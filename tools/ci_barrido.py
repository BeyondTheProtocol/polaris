...
# Remitentes de boletines de empresa (22-sep-26: `updates@glass.health` en un comentario de
# correo.py puso el CI público en rojo). SOLO coincidencia exacta y fuera de webmails: con los
# prefijos de LOCAL_OK, «news.x7k2q@gmail.com» habría pasado por buzón de servicio.
LOCAL_SERVICIO = {"updates", "newsletter", "news", "notifications", "notificaciones"}
WEBMAIL = re.compile(r"^(?:gmail|googlemail|hotmail|outlook|live|yahoo|icloud|me|proton(?:mail)?)\.",
                     re.I)
...
def _email_de_nadie(addr):
    ...
    if local in LOCAL_OK or local.endswith("team") or any(
        ...
        return True
    if local in LOCAL_SERVICIO and not WEBMAIL.search(dom):
        return True
...
def selftest():
    ...
    limpios = ("un comentario normal sobre oncología y metástasis",
               ...
               "boletín de updates@glass.health")
    for texto in limpios:
        if revisar_texto(texto):
            print("✗ falso positivo en %r → %s" % (texto, revisar_texto(texto))); fallos += 1
    # Local sin nombre propio: el despersonalizador del espejo cambiaba «contacto» por «contacto»
    # y el caso, ya en el árbol público, caía en otra regla (CI rojo, 22-sep).
    for texto in ("news.x7k2q@gmail.com", "updates@gmail.com"):     # servicio ≠ persona en webmail
        if not revisar_texto(texto):
            print("✗ deja pasar %r como buzón de servicio" % texto); fallos += 1
