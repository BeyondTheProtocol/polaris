...
def numeros(texto, lengua="es"):
    ...
    t = re.sub(r"[\u2010-\u2015\u2212]", "-", _CITA.sub(" ", texto or ""))   # guiones Unicode
    if lengua == "es":
        t = _RUIDO.sub(" ", t)
...
def texto_completo_oa(tipo, ident):
    ...
    if code != 200 or not xml:
        return None
    return r[0]["pmcid"], re.sub(r"<[^>]+>", " ", xml)


def soporte(afirmacion, cita, fetch=None, texto_completo=None):
...
def _rc(res):
    return 2 if any(r["estado"] == NO_RESPALDA for r in res) else 0
...
def main(argv):
    ...
    res = [soporte(frase, c) for c in args[1:]]
    if como_json:
        print(json.dumps(res, ensure_ascii=False))
    else:
        for r in res:
