...
def construir_digest(canal="local"):
    ...
    if data["fallos"]:
        lineas.append("🛠️ %d job(s) del lazo caídos (queue/failed)." % len(data["fallos"]))
...
def _cargar_nombres_deny():
    try:
        with io.open(_NOMBRES_LOCAL, encoding="utf-8") as fh:
            d = json.load(fh)
    except (OSError, ValueError):
        return frozenset()
    return frozenset(str(x).lower() for x in d.get("nombres", []) if str(x).strip())
