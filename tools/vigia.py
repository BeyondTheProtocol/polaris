...
# 20-sep-2026: decían «arréglalo», y eso es IMPOSIBLE en el lazo — el muro no deja escribir
# código en NINGÚN perfil (`.claude/hooks/muro_guard.py`, WRITE_EXEC_EXT). El agente chocaba
# contra la pared, respondía en prosa y el job se cerraba como hecho: gasto real, cero arreglo.
# Mismo cambio que hizo `healthcheck._encolar_investigacion` el 31-jul por la misma razón.
# Pedirle lo imposible no es exigencia, es un bucle de gasto.
_TAREA_PARA_CODIGO = {
    "respuesta_floja": "El bot dio una respuesta floja (no encontró datos o no respondió bien). "
                       "Averigua POR QUÉ y déjalo escrito.",
    "bot_inestable": "El bot de Telegram se reinicia en bucle con errores reales. Diagnostica el "
                     "traceback en los logs y déjalo escrito.",
    "error_log": "Apareció un error nuevo en los logs del bot/dispatcher. Diagnostícalo y déjalo "
                 "escrito.",
}
_COMO_ENTREGAR = (
    "\n\nTu trabajo aquí es DIAGNOSTICAR y DEJARLO ESCRITO, no tocar código: corres en modo "
    "autónomo y el muro te bloquea (correctamente) editar cualquier ejecutable. Si el arreglo "
    "exige tocar código, ese arreglo es de una sesión con {{TITULAR}}, no tuyo.\n"
    "Apunta la causa en el libro: `python3 tools/deuda.py abrir \"%s\" \"<la causa en una línea>\"` "
    "(si ya estaba, `python3 tools/deuda.py nota \"%s\" \"<lo nuevo>\"`). Sin esa anotación el "
    "encargo NO se da por entregado."
)
...
def _encolar_para_codigo(nuevas):
    ...
    for a in nuevas:
        if a.get("clase") != "necesita_codigo":
            continue
        tipo = a.get("tipo", "?")
        base = _TAREA_PARA_CODIGO.get(tipo, "Anomalía del vigía. Averigua qué pasa y déjalo escrito.")
        clave_deuda = "vigia:%s" % (a.get("key") or tipo)
        intencion = ("[vigía] %s\n\nDetalle: %s\n(clave: %s)%s"
                     % (base, a.get("detalle", ""), a.get("key", ""),
                        _COMO_ENTREGAR % (clave_deuda, clave_deuda)))
        try:
            import cola
            jid = cola.enqueue(intencion, prioridad="normal", procedencia="vigia",
                               agente="tecnico", tipo="exec",
                               prueba={"tipo": "deuda", "clave": clave_deuda})
