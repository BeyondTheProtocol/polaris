...
un worktree, porque en vivo su `ask` se resolvía solo (ver la regla). El worktree se detecta
...
def _push_de_casa_base(cmd):
    ...
    deuda que lo citaba, y en el flujo de PRs de la web (`~/projects/…`) el push es legítimo. El
    troceo del shell es el de `salida_guard.py` —una sola definición, no dos que diverjan—. Si
    no se puede importar, se vuelve al regex de siempre: ante la duda, recordar de más."""
    if "push" not in cmd:
        return False
    try:
        ...
        ordenes = sg._ordenes(cmd)
    except Exception:
        return bool(re.search(r"\bgit\s+push\b", cmd))
    base = os.path.realpath(_casa_base())
    dirs = os.path.abspath(_CWD) if _CWD else os.getcwd()
    for pal, _cuerpo in ordenes:
        if not pal:
            continue
        prog, args = os.path.basename(pal[0]), pal[1:]
        if prog == "cd":
        ...
        if args and args[0] == "push":
            ...
            if r == base or r.startswith(base + os.sep):
                return True
    return False
