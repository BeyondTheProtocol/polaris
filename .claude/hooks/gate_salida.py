...
def pendientes_sin_verificar(t, tools=None):
    ...
    «Qué espera tu OK» salió del patrón el 25-sep-26: es el cierre obligatorio de la constitución
    y lista lo que ESTE turno deja a su firma, no pendientes de memoria. Daba 50 de 76 disparos en
    el replay de 30 días. Lo que pide OK ya lo vigila `gestion_pide_ok`.
...
def no_se_sin_mirar(t, tools=None):
    ...
    «no sé» exige la tilde desde el 25-sep-26: con `s[ée]` casaba cualquier pasiva refleja
    («no se ha fusionado», «no se toca») y el check saltaba en el 29 % de los turnos (754 → 108
    disparos en el replay de 30 días). Idea de {{CONTACTO}} (https://contacto), con su
    agente KAI, revisión del 25-sep-2026.
    """
    if tools is None:
        return None
    if not re.search(r"(no sé\b|no tengo ese dato|no me consta|no lo encuentro)", t, re.I):
        return None
...
# cuatro son las mecanizables más cercanas al muro y a NED. Entran en `aviso` (campo `modo` propio
# en normas.json); la subida a bloqueo sigue la escalera del gate, con datos de `replay_gate.py`.

# Un borrador hacia fuera se reconoce por su envoltorio: bloque ``` o cita en «>». La prosa de la
# conversación conmigo puede nombrar {{CONTACTO}} o hablar de la recaudación; lo que no puede es meterlo
# en el texto que va a salir.
_OUTWARD = (r"(post|tuit|tweet|\bX\b|linkedin|instagram|\bIG\b|reel|stor(y|ies)|caption|pie de foto|"
            r"copy|web|p[áa]gina|publica|borrador|correo|email|mensaje|DM|whatsapp|nota de prensa|"
            r"pitch|comentario|respuesta p[úu]blica)")


def _borradores(t):
    """Trozos de la respuesta que tienen forma de texto para fuera: bloques ``` y citas «>»,
    solo si la respuesta habla de algo que sale (post, correo, web…)."""
    if not re.search(_OUTWARD, _sin_fences(t), re.I):
        return []
    # Bloques con lenguaje de código declarado (```bash, ```json…) no son texto para una persona.
    trozos = [c for lang, c in re.findall(r"```([^\n]*)\n(.*?)```", t, flags=re.S)
              if not re.match(r"\s*(bash|sh|zsh|shell|console|python|py|json|ya?ml|js|ts|html|"
                              r"css|diff|sql|toml|ini|text\b)", lang, re.I)]
    cita, actual = [], []
    for linea in t.splitlines():
        if linea.lstrip().startswith(">"):
            actual.append(linea.lstrip()[1:])
        elif actual:
            cita.append("\n".join(actual))
            actual = []
    if actual:
        cita.append("\n".join(actual))
    # Un bloque de código de verdad (shell, python, json) no es un borrador.
    return [x for x in trozos + cita
            if x.strip() and not re.search(r"^\s*(\$ |python3|node |import |def |\{|git |cd |<\w)|"
                                           r"~/|/Users/|https?://localhost", x, re.M)]


def _norm_lex(t):
    """Minúsculas, sin tildes ni caracteres invisibles: que «Oluné» o «O​lune» no se cuelen."""
    import unicodedata
    t = re.sub("[​‌‍⁠﻿]", "", t.lower())
    return "".join(c for c in unicodedata.normalize("NFKD", t) if not unicodedata.combining(c))


def muro_en_borrador(t, tools=None):
    """feedback-muro-sobre-comite — el muro manda sobre cualquier comité: el copy que me devuelve
    un agente se re-lee contra él antes de dárselo a {{TITULAR}}. Parte léxica, la única que una regex
    puede ver: ni «{{CONTACTO}}» ni «ingeniera» en un texto que va a salir (CLAUDE.md, «Privado por
    defecto»). «ingeniero» genérico («artículo ingeniero») no canta: el veto es llamarla a ella."""
    for b in _borradores(t):
        n = _norm_lex(b)
        if "contacto" in re.sub(r"\s+", "", n):
            return ("Un borrador para fuera nombra «{{CONTACTO}}». El muro manda sobre el comité que lo "
                    "redactó: fuera antes de enseñárselo.")
        m = re.search(r"\b(soy|como|una|la) ingeniera\b|\bcientifica (que|e ingeniera)", n)
        if m:
            return ("Un borrador para fuera la llama «ingeniera» («%s»). En público es «ingeniera» "
                    "+ lo que construye." % m.group(0))
    return None


def importes_recaudacion(t, tools=None):
    """feedback-no-importes-recaudacion-fuera — ni cifras de la recaudación ni la vía de donación
    en un mensaje hacia fuera: riesgo con las ayudas sociales y munición para el acosador. Nombrar
    la vía (GoFundMe…) necesita antes el OK de legal-burocracia."""
    dinero = r"(\d[\d.,]*\s?(€|euros?|k€|mil euros|EUR)|€\s?\d)"
    via = r"(gofundme|go fund me|paypal\.me|bizum|recaudad|recaudaci|llevamos recogid|donaciones)"
    for b in _borradores(t):
        if re.search(via, b, re.I) and re.search(dinero, b, re.I):
            return ("Un borrador para fuera lleva un importe junto a la recaudación o la vía de "
                    "donación. Quita la cifra; nombrar la vía necesita OK de legal-burocracia.")
        if re.search(r"gofundme|go fund me|paypal\.me", b, re.I):
            return ("Un borrador para fuera nombra la vía de donación. Si va a ser público, antes "
                    "necesita el OK de legal-burocracia.")
    return None


# «GPT-5» a secas es un modelo (sale en benchmarks); el buscador es ChatGPT o Deep Research.
_BUSCADORES_IA = r"(perplexity|grok|gemini|chatgpt|chat gpt|deep ?research|you\.com|copilot)"


def cita_ia_buscador(t, tools=None):
    """feedback-evidencia-grok-no-perplexity — lo que devuelve un buscador con IA (Grok, Perplexity,
    Gemini…) es una PISTA, no una fuente: su tasa de citas inventadas es alta (CJR 2026, en la
    propia memoria). Canta si le paso un dato atribuido a uno de ellos sin decir que está sin
    verificar ni haberlo cotejado con la fuente primaria."""
    for f in re.split(r"(?<=[.\n])\s+", _sin_fences(t)):
        if not re.search(_BUSCADORES_IA, f, re.I):
            continue
        atribuye = (re.search(r"(seg[úu]n|dice|afirma|encontr[óo]|devuelve|devolvi[óo]|report[aó]|"
                              r"cita|sostiene|me da|trae)\b[^.\n]{0,50}" + _BUSCADORES_IA, f, re.I)
                    or re.search(_BUSCADORES_IA + r"[^.\n]{0,20}\b(dice|afirma|encontr[óo]|"
                                 r"devuelve|devolvi[óo]|report[aó]|cita|sostiene|trae)\b", f, re.I))
        if not atribuye:
            continue
        # Con dato: cifra, porcentaje, ensayo o cita. Sin dato es charla sobre la herramienta.
        if not re.search(r"\d+\s?%|\bNCT\d|\bPMID|10\.\d{4}/|\b(ensayo|estudio|paper|art[íi]culo|"
                         r"mediana|supervivencia|respuesta)\b", f, re.I):
            continue
        if _sello(f) or re.search(r"pista|no fiable|cotej|contrast|fuente primaria|comprob|"
                                  r"alucin|invent", f, re.I):
            continue
        return ("Pasas un dato atribuido a un buscador con IA («%s…») sin sello. Es una pista: "
                "va «sin verificar» o cotejado contra la fuente primaria." % f.strip()[:110])
    return None


def consenso_lentes(t, tools=None):
    """feedback-acuerdo-entre-lentes-no-es-evidencia — N agentes del mismo modelo con prompts
    hermanos son UN testigo repetido. Si digo que coinciden, digo cuántos eran independientes."""
    sujeto = (r"(las|los|sus)?\s*(\d+|dos|tres|cuatro|cinco|seis|siete|todas las|todos los)\s+"
              r"(lentes|agentes|comit[ée]s|revisores|consejeros|expertos|jueces|red-?teams?)")
    # Sin «modelos» (replay 25-sep-26): cinco clasificadores de imagen distintos SÍ son testigos
    # independientes; la norma va de agentes LLM hermanos.
    acuerdo = r"(coinciden|convergen|est[áa]n de acuerdo|un[áa]nim|lo confirman|dicen lo mismo)"
    for f in re.split(r"(?<=[.\n])\s+", _sin_bloques(t)):
        # Solo «sujeto … acuerdo»: el orden inverso cazaba «coinciden; dos agentes suben a opus».
        if not re.search(sujeto + r"[^.\n|]{0,40}" + acuerdo, f, re.I):
            continue
        # Correcto cuando ya lo calibro: independencia, mismo modelo, un testigo, o lo niego.
        if re.search(r"independient|mismo modelo|un (solo )?testigo|no multiplica|no suma|"
                     r"no cuenta|no (es|son) evidencia|\bno\b[^.\n]{0,10}(coinciden|convergen)|"
                     r"discrep|salvo|excepto|menos\b", f, re.I):
            continue
        return ("«%s…»: que coincidan agentes del mismo modelo no multiplica la confianza. Di "
                "cuántos eran independientes de verdad (otro modelo, otra fuente, un humano)."
                % f.strip()[:110])
    return None
...
def _al_vencer(texto, segundos):
    """Lo decide el proceso vigilante cuando el gate no termina a tiempo (ver main)."""
    if _ids_cita(texto):
        sys.stderr.write("GATE DE SALIDA — no me dio tiempo (%.1f s) a verificar las citas de esta "
                         "respuesta. Verifícalas o márcalas «sin verificar» antes de entregar.\n"
                         % segundos)
        return 2
    print(json.dumps({"systemMessage": "⚠️ Gate de salida: no terminó en %.1f s; esta respuesta "
                                       "salió SIN revisar." % segundos}, ensure_ascii=False))
    return 0
...
def main():
    ...
    # Watchdog (25-sep-26). Idea de {{CONTACTO}} (https://contacto), con su agente KAI,
    # revisión del 25-sep-2026. Si tardo más que mi timeout de settings (8 s), Claude Code me
    # cancela y la respuesta sale sin revisar y sin que nadie lo sepa. El gate sigue siendo
    # FAIL-OPEN para el estilo, pero ya no en silencio; y si la respuesta trae citas, frena UNA
    # vez (con `stop_hook_active` no hay bucle), porque citas_fabricadas es el check que bloquea
    # siempre y quedarse sin tiempo no puede equivaler a «cita verificada».
    sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
    try:  # sin el vigilante el guard sigue siendo el guard: su falta no lo tumba
        import _watchdog
    except ImportError:
        _watchdog = None
        sys.stderr.write("gate_salida: falta _watchdog.py; sigo sin vigilante\n")
    if _watchdog:
