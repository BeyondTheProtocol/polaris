...
2. **Limpia solo lo de CERO pérdida (autónomo) — SIEMPRE por `tools/ramas.py`, nunca git crudo:**
   - Usa **`python3 tools/ramas.py limpia --si`**: poda worktrees muertos + borra ramas **limpias y vacías** (sin commits propios ni sesión viva), y lo hace **bajo el candado compartido `git-mutex`** (serializado con `cerrar_sesion.py`). Equivale a `git branch -d` (rechaza lo no fusionado, **jamás `-D`**) + `git worktree prune`, pero SIN correr en paralelo con una fusión de otra sesión.
