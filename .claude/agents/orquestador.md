...
- **Verificación obligatoria:** `verificacion` contrasta cada cifra/cita contra fuente (PRISMA donde haya literatura) **antes** de que el acta llegue a {{TITULAR}}. El veredicto red-team de `verificacion` lo comprueba **`herramientas-medicas`**: si lo comprobara un agente al que `verificacion` ya ha comprobado, el tool lo rechaza (comprobación mutua, juez y parte).
- **Registra el debate:** junta los veredictos en JSON y corre `python3 tools/decision_alto_riesgo.py acta --in panel.json --guardar --etiqueta "<tema>"`. El tool exige ≥2 **agentes** distintos del gabinete (dos lentes del mismo agente cuentan como una), que TODAS estén verificadas (con informe de la bóveda, la `comprobacion` lleva `fragmento` literal y el tool lo busca dentro del fichero), marca dónde discrepan y da la confianza final; **fail-closed** (rc 1 = panel incompleto / sin verificar / **discrepancia abierta** → no se presenta como consenso, va a {{TITULAR}} y sus médicas CON el debate). El acta queda auditable en `tools/state/decisiones_alto_riesgo/`.
